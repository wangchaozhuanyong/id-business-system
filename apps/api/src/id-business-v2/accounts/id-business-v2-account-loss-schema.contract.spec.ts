...
describe('ID Business V2 account loss schema contract', () => {
  ...
  it('keeps one immutable loss snapshot per account and ledger entry', () => {
    ...
    expect(migration).toContain('id_business_v2_accounts_permanent_loss');
    expect(migration).toContain('OLD.loss_reported_at IS NOT NULL');
    expect(migration).toContain('NEW.current_balance <> 0::numeric');
    expect(migration).toContain('loss_record.account_id = NEW.id');
  ...
  it('allows only standalone debit loss entries, including a zero-value loss snapshot', () => {
