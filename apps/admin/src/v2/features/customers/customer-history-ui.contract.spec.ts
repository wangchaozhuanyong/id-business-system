...
describe('customer history and contact UI contract', () => {
  it('shows policy-aware contacts on desktop and mobile', () => {
    ...
    expect(list).toContain('row.displayWhatsapp');
    expect(list).toContain('row.displayPhone');
  ...
  it('explains the keep and clear behavior for stored WhatsApp', () => {
    ...
    expect(sensitiveAccessDialog).toContain('page.revealFieldLabel');
    expect(sensitiveAccessDialog).toContain('page.revealContact');
  });
});
