...
- 监控每 5 分钟检查一次。最新成功备份超过 20 分钟、任务失败、文件无效或超过容量上限时显示
  macOS 通知；同时检查专用备份角色的空闲事务超时为 2 分钟、空闲会话超时为 16 分钟，且不存在
  超过 16 分钟的 `idle in transaction` 遗留连接。`pg_dump` 会清零空闲事务超时，16 分钟会话超时
  高于单命令 15 分钟硬上限，用于在异常进程退出后兜底清理，同时不会中断正常大表 `COPY`。
  Session Pooler 明确报告连接中断时，脚本会删除未完成文件并整份重新导出，最多尝试 3 次；权限、
  数据或校验错误不会重试。相同告警 1 小时内不重复弹出。
...
生产 migration 会在执行 Prisma 前检查 `id_v2_backup` 事务。正常备份尚未完成时最多等待 3 分钟；
等待超过 3 分钟会在 Prisma 写入 migration 记录前停止。该门禁不会自动终止数据库连接；超过
16 分钟的遗留连接会触发备份监控告警，确认对应备份进程已结束后再由管理员处理。
