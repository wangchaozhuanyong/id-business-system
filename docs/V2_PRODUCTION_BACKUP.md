...
- 监控每 5 分钟检查一次。最新成功备份超过 20 分钟、任务失败、文件无效或超过容量上限时显示
  macOS 通知；同时检查专用备份角色的空闲事务超时必须为 2 分钟，且不存在超过 2 分钟的
  `idle in transaction` 遗留连接。备份命令还固定设置 `idle_session_timeout=2min`，防止 `pg_dump`
  清零空闲事务超时后，连接池中的断连会话继续持锁。相同告警 1 小时内不重复弹出。
...
超过 2 分钟的空闲事务或等待超时都会在 Prisma 写入 migration 记录前停止。该门禁不会自动终止
数据库连接，遗留连接必须在确认对应备份进程已结束后由管理员处理。
