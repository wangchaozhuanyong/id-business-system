...
- 自动充值页面仅保留右上角“代理 IP 与窗口设置”入口；当前代理模式、代理来源脱敏摘要、窗口分组和语言显示在设置抽屉顶部。
...
| 同步选项   | 标签页、Cookie、本地存储同步    | 均关闭                         | 登录态保护固定关闭，旧配置自动兼容   |
...
- `apps/admin/src/v2/features/auto-recharge/V2AutoRechargeView.vue`、`auto-recharge.css`：主页面右上角的唯一设置入口；配置摘要移入设置抽屉，保留上一轮账单邮箱修复。
...
## 慢代理加载与窗口重试（本地实现，2026-09-13）

- 设置抽屉增加每轮等待时间（1–10 分钟，默认 2）和最多重建次数（0–2 次，默认 2）。沿用 `browser_options` JSON，旧记录缺失新键时补默认值，不增加迁移。
- 本机首次官网导航、会话读取及账户核对共用等待预算；正常慢加载不再被原 45 秒导航或 25 秒读取限制提前结束。验证码人工等待不计入加载时间，恢复后继续核对。
- 仅初始会话失败且本轮建单、付款请求均为零时允许重建；会话一旦核实，后续异常不能重新进入窗口重试。一个任务最多尝试三个窗口，重用配置和内存凭据，不重新提交充值任务。
- 仅关闭本任务创建的窗口 ID，确认进程退出后调用单窗口删除。清理状态不能确认时停止；三次都失败则清理最后窗口并报告原因。取消期间不继续创建或删除窗口。
- 回传尝试次数、等待秒数、加载步骤及脱敏异常。每轮进度及人工验证等待续期 45 分钟；收到停止指令后中断加载，网页须得到本机取消确认后才能结束服务端任务。
- 连接器健康能力新增 `session-load-retry`，旧连接器在预检阶段提示升级。原协议版本仍为 2，升级时需同步完整 worker 模块，包含新增 `browser_session.py` 和 `bitbrowser_retry.py`。
- 本地验证：前端 37 项、API 53 项、Python 定向 33 项通过；前端/共享包/API 构建、定向 lint、中文文案、金额规范检查通过；1440/768/390px 模拟设置保存回显及加载进度、停止入口通过。
- 未运行全库 CI，未修改生产配置、推送、部署或升级本机已安装连接器；未创建真实窗口或付款。检查脚本首次参数错误与浏览器测试缓存路径问题已修正，仅补验未通过或新增部分。
