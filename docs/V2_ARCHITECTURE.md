...
中转。CurrencyAPI Provider 请求 `base_currency=CNY`，将“1 CNY 可兑换外币数量”以共享 Decimal/BigInt
算法取倒数后计算收购价，全链路最多 8 位汇率精度，不使用 JavaScript 浮点数参与业务金额计算。

自动收购汇率复用受 Bearer 密钥保护的数据库 Cron 入口，新增 `5 * * * *` 调度，并以数据库部分唯一索引
