...
项目现有 Node 运行时，并让 Cloudflare 的 `API_UPSTREAM_BASE_URL` 指向该 Node API 根域名。
前端继续使用同源 `/api`。未经单独设计和验收，不得把应用专用密码转发给外部邮件代查服务。

生产发布前必须确认 Node 运行时未设置 `SUPABASE_EDGE_FUNCTION=true`，服务器允许出站
TCP 993，并且 `/api/health/ready` 已经返回 200。
...
- 运行日志只记录供应商、结果、耗时、错误码和请求标识，不记录邮箱、查询码、应用专用密码或邮件内容。

## 生产监控

- `/api/health/ready` 连续两次非 200 时告警。
- 五分钟内 `provider_error` 超过查询量的 20% 或公开查询 5xx 超过 5% 时告警。
- 邮箱查询 P95 耗时超过 12 秒时告警；`rate_limited` 比例异常升高时检查滥用或代理 IP 配置。
- 发布后至少观察 24 小时，期间不得在日志平台上开启请求体采集。
